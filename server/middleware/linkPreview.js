const userAgentRedirect = (req, res, next) => {
  global.logger.info({ message: `USER AGENT REDIRECT MIDDLEWARE. REQ: ${req.url}, USER AGENT: ${req.headers['user-agent']}`, level: 7, timestamp: new Date().toISOString(), userID: 0 });
  ...
  if (mobileUserAgents.some((regex) => regex.test(userAgent))) {
    global.logger.info({ message: `REDIRECT LINK FOR MOBILE: ${redirectLink}`, level: 7, timestamp: new Date().toISOString(), userID: 0 });
    ...
    global.logger.info({ message: `${req.headers['user-agent']} USER REQUEST, REDIRECTING TO ${redirectLink}`, level: 6, timestamp: new Date().toISOString(), userID: 0 });
  ...
  if (isAllowedUserAgent(userAgent)) {
    global.logger.info({ message: `${req.headers['user-agent']} ALLOWED, SENDING TO GET PREVIEW DETAILS`, level: 6, timestamp: new Date().toISOString(), userID: 0 });
    next();
  } else {
    // else drop the request
    global.logger.info({ message: `${req.headers['user-agent']} NOT USER OR LINK PREVIEW CRAWLER, DROP REQUEST`, level: 6, timestamp: new Date().toISOString(), userID: 0 });
