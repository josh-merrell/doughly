...
const getHtml = async (url) => {
  ...
  try {
    ...
    throw errorGen(err.message || '*scraper-getHtml* Unhandled Error', err.code || 520, err.name || 'unhandledError_scraper-getHtml', err.isOperational || false, err.severity || 2); //message, code, name, operational, severity
  } finally {
    if (browser) {
      await browser.close();
...
const extractFromHtml = async (html) => {
  try {
    ...
    throw errorGen(err.message || '*scraper-extractFromHtml* Unhandled Error', err.code || 520, err.name || 'unhandledError_scraper-extractFromHtml', err.isOperational || false, err.severity || 2); //message, code, name, operational, severity
  }
};
