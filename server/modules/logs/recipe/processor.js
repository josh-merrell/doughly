...
module.exports = ({ db }) => {
  ...
  async function create(options) {
    ...
    const { data: log, error } = await db.from('recipeLogs').insert({ recipeLogID: customID, userID, subjectID, associatedID, eventType, oldValue, newValue, message, createdTime }).select('*').single();

    if (error) {
      global.logger.info(`Error creating recipeLog: ${error.message}`);
      return { error: error.message };
    }
    global.logger.info(`Created recipeLog`);
    return {
      logID: log.recipeLogID,
    };
  }
