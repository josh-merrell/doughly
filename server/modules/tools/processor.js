...
module.exports = ({ db }) => {
  async function getAll(options) {
    ...
    try {
      ...
      if (error) {
        throw errorGen(`*tools-getAll* Error getting tools: ${error.message}`, 511, 'failSupabaseSelect', true, 3);
      }
      ...
      throw errorGen(err.message || '*tools-getAll* Unhandled Error', err.code || 520, err.name || 'unhandledError_tools-getAll', err.isOperational || false, err.severity || 2);
    }
  }
  ...
  async function getByID(options) {
    ...
    try {
      ...
      if (error) {
        throw errorGen(`*tools-getByID* Error getting tool: ${error.message}`, 511, 'failSupabaseSelect', true, 3);
      }
      global.logger.info({ message: `*tools-getByID* Got tool`, level: 6, timestamp: new Date().toISOString(), userID: userID || 0 });
      return tool;
    } catch (err) {
      throw errorGen(err.message || '*tools-getByID* Unhandled Error', err.code || 520, err.name || 'unhandledError_tools-getByID', err.isOperational || false, err.severity || 2);
    }
  }
  ...
  async function create(options) {
    ...
    try {
      ...
      if (existingToolError) {
        throw errorGen(`*tools-create* Error getting existing tools: ${existingToolError.message}`, 511, 'failSupabaseSelect', true, 3);
      }
      if (existingTool.length > 0) {
        throw errorGen(`*tools-create* Tool with name ${name} already exists, cannot create tool`, 515, 'cannotComplete', false, 3);
      }
      ...
      if (error) {
        throw errorGen(`*tools-create* Error creating tool: ${error.message}`, 512, 'failSupabaseInsert', true, 3);
      }
      ...
      throw errorGen(err.message || '*tools-create* Unhandled Error', err.code || 520, err.name || 'unhandledError_tools-create', err.isOperational || false, err.severity || 2);
    }
  }
  ...
  async function update(options) {
    ...
    try {
      ...
      if (existingToolError) {
        throw errorGen(`*tools-update* Error checking for existing tool: ${existingToolError.message}`, 511, 'failSupabaseSelect', true, 3);
      }
      if (existingTool.length === 0) {
        throw errorGen(`*tools-update* Tool with ID ${toolID} does not exist, cannot update tool`, 515, 'cannotComplete', false, 3);
      }

      //validate that the provided name is not already used by another tool
      const { data: existingTool2, error: existingToolError2 } = await db.from('tools').select().eq('name', name);
      if (existingToolError2) {
        throw errorGen(`*tools-update* Error checking for duplicated tool name: ${existingToolError2.message}`, 511, 'failSupabaseSelect', true, 3);
      }
      if (existingTool2.length) {
        throw errorGen(`*tools-update* Tool with name ${name} already exists, cannot create tool`, 515, 'cannotComplete', false, 3);
      }
      ...
      throw errorGen(err.message || '*tools-update* Unhandled Error', err.code || 520, err.name || 'unhandledError_tools-update', err.isOperational || false, err.severity || 2);
    }
  }
  ...
  async function deleteTool(options) {
    ...
    try {
      ...
      if (existingToolError) {
        throw errorGen(`*tools-deleteTool* Error checking for existing tool: ${existingToolError.message}`, 511, 'failSupabaseSelect', true, 3);
      }
      if (!existingTool.length) {
        throw errorGen(`*tools-deleteTool* Tool with ID ${toolID} does not exist, cannot delete tool`, 515, 'cannotComplete', false, 3);
      }
      ...
      if (toolError) {
        throw errorGen(`*tools-deleteTool* Error getting related recipeTools for tool to delete: ${toolID} : ${toolError.message}`, 511, 'failSupabaseSelect', true, 3);
      }
      ...
      for (let i = 0; i < relatedRecipeTools.length; i++) {
        ...
        if (recipeToolDeleteResult.error) {
          throw errorGen(`*tools-deleteTool* Error deleting recipeToolID: ${relatedRecipeTools[i].recipeToolID} prior to deleting tool ID: ${toolID} : ${recipeToolDeleteResult.error}`, 514, 'failSupabaseDelete', true, 3);
        }
      ...
      if (toolStockError) {
        throw errorGen(`*tools-deleteTool* Error getting related recipeTools for tool to delete: ${toolID} : ${toolStockError.message}`, 511, 'failSupabaseSelect', true, 3);
      }
      ...
      for (let i = 0; i < relatedToolStocks.length; i++) {
        ...
        if (toolStockDeleteResult.error) {
          throw errorGen(`*tools-deleteTool* Error deleting toolStockID: ${relatedToolStocks[i].toolStockID} prior to deleting tool ID: ${toolID} : ${toolStockDeleteResult.error}`, 514, 'failSupabaseDelete', true, 3);
        }
      ...
      if (error) {
        throw errorGen(`*tools-deleteTool* Error deleting tool: ${error.message}`, 514, 'failSupabaseDelete', true, 3);
      }
      //add a 'deleted' log entry
      createKitchenLog(userID, authorization, 'deleteTool', Number(toolID), null, null, null, `Deleted Tool: ${existingTool[0].name}`);
      return { success: true };
    } catch (err) {
      throw errorGen(err.message || '*tools-deleteTool* Unhandled Error', err.code || 520, err.name || 'unhandledError_tools-deleteTool', err.isOperational || false, err.severity || 2);
    }
  }
