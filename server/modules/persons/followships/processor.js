...
module.exports = ({ db, dbPublic }) => {
  async function getAll(options) {
    ...
    try {
      ...
      if (error) {
        throw errorGen(`*followships-getAll* Error getting followships: ${error.message}`, 400, 'failSupabaseSelect', true, 3);
      }
      ...
      if (name) {
        for (let i = 0; i < followships.length; i++) {
          ...
          if (profileError) {
            throw errorGen(`*followships-getAll* Error getting profile user_id ${followship[i].following}: ${profileError.message}`, 511, 'failSupabaseSelect', true, 3);
          }
      ...
      throw errorGen(err.message || '*followships-getAll* Unhandled Error', err.code || 520, err.name || 'unhandledError_followships-getAll', err.isOperational || false, err.severity || 2);
    }
  }
  ...
  async function getAllFollowers(options) {
    ...
    try {
      ...
      const { data: followships, error } = await q;
      if (error) {
        throw errorGen(`*followships-getAllFollowers* Error getting follower followships: ${error.message}`, 511, 'failSupabaseSelect', true, 3);
      }
      ...
      throw errorGen(err.message || '*followships-getAllFollowers* Unhandled Error', err.code || 520, err.name || 'unhandledError_followships-getAllFollowers', err.isOperational || false, err.severity || 2);
    }
  }
  ...
  async function getFollowshipByID(options) {
    try {
      ...
      if (error) {
        throw errorGen(`*followships-getFollowshipByID* Error getting followship ${options.followshipID}: ${error.message}`, 511, 'failSupabaseSelect', true, 3);
      }
      ...
      throw errorGen(err.message || '*followships-getFollowshipByID* Unhandled Error', err.code || 520, err.name || 'unhandledError_followships-getFollowshipByID', err.isOperational || false, err.severity || 2);
    }
  }
  ...
  async function create(options) {
    ...
    try {
      ...
      if (profileError) {
        throw errorGen(`*followships-create* Error getting profile user_id ${following}: ${profileError.message}`, 511, 'failSupabaseSelect', true, 3);
      }
      if (!profile) {
        throw errorGen(`*followships-create* Profile user_id ${following} does not exist, can't create followship`, 515, 'cannotComplete', false, 3);
      }
      ...
      if (existingFollowshipError) {
        throw errorGen(`*followships-create* Error checking for existing followship: ${existingFollowshipError.message}`, 511, 'failSupabaseSelect', true, 3);
      }
      if (existingFollowship.length && existingFollowship[0].deleted === false) {
        throw errorGen(`*followships-create* Followship already exists, cannot create`, 515, 'cannotComplete', false, 3);
        ...
        if (undeleteError) {
          throw errorGen(`*followships-create* Error undeleting followship ${existingFollowship[0].followshipID}: ${undeleteError.message}`, 513, 'failSupabaseUpdate', true, 3);
        }
      ...
      if (error) {
        throw errorGen(`*followships-create* Error creating followship: ${error.message}`, 512, 'failSupabaseInsert', true, 3);
      }
      ...
      throw errorGen(err.message || '*followships-create* Unhandled Error', err.code || 520, err.name || 'unhandledError_followships-create', err.isOperational || false, err.severity || 2);
    }
  }
  ...
  async function deleteFollowship(options) {
    try {
      ...
      if (error) {
        throw errorGen(`*followships-deleteFollowship* Error getting followship ${options.followshipID}: ${error.message}`, 511, 'failSupabaseSelect', true, 3);
      }
      if (!followship) {
        throw errorGen(`*followships-deleteFollowship* Followship ${options.followshipID} does not exist, cannot delete`, 515, 'cannotComplete', false, 3);
      }

      // delete followship
      const { data, error: deleteError } = await db.from('followships').update({ deleted: true }).eq('followshipID', options.followshipID);
      if (deleteError) {
        throw errorGen(`*followships-deleteFollowship* Error deleting followship ${options.followshipID}: ${deleteError.message}`, 514, 'failSupabaseDelete', true, 3);
      }
      ...
      throw errorGen(err.message || '*followships-deleteFollowship* Unhandled Error', err.code || 520, err.name || 'unhandledError_followships-deleteFollowship', err.isOperational || false, err.severity || 2);
    }
  }
