...
export class StringsService {
  public readonly onboardingStrings = {
    ...
    subscribeRecipeModal: `To save a public recipe to your own account, map each ingredient and tool to your own options (we started you with a few) or choose "Copy to Kitchen". Doughly keeps track of all units and inventory behind the scenes, so we'll need some converions as well. Our AI will offer suggestions, but you can always adjust them.`,
    needFinishSubscription: `Finish subscribing to this recipe to continue. You can remove it later if you prefer not to keep it.`,
    subscribeRecipePage: `We made it! This copy of the Public recipe is now connected to your kitchen. By default, the "Shopping" button will not check your current inventory when preparing a shopping list for this recipe. You can enable that powerful feature in the Settings page. For now, Explore the "Make Now!" button, then navigate to "Social".`,
    ...
    shoppingPageOverview: `To prep for a new grocery run, try adding the recipe you just subscribed to. Once added, you'll see all the ingredients appear in the list. You can also add non-recipe items manually.`,
