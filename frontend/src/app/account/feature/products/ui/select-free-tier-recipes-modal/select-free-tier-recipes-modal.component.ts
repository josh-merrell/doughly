...
@Component({
  selector: 'dl-select-free-tier-recipes-modal',
  standalone: true,
  imports: [
    CommonModule,
    RecipeCardComponent,
  ...
  templateUrl: './dl-select-free-tier-recipes-modal.component.html',
})
export class SelectFreeTierRecipesModalComponent {
  ...
  remainingSelectionCount = computed(() => {
    ...
    const maxSelectionCount = this.productService.licences.recipeCreateLimit;
  ...
  ngOnInit() {
    ...
    this.store.select(selectRecipes).subscribe((items) => {
      // filter out any that have type "subscription"
      const recipes = items.filter((recipe) => recipe.type !== 'subscription');
      const IDs = recipes
  ...
  onSubmitClick() {
    if (
      ...
      this.recipeService
        .setFreeTierCreated(this.selectedRecipeIDs())
        .subscribe((error) => {
